defmodule Election.Elector do
  ...
  def start_link(%{name: name} = opts) when not is_nil(name),
    do: GenServer.start_link(@name, opts, name: name)

  def start_link(opts), do: GenServer.start_link(@name, opts, name: @name)

  @doc false
  def init(opts) do
    {:ok, %{leader: nil, config: Enum.into(opts, %{})},
     Access.get(opts, :first_election_in, @default_first_election_in)}
  end
  ...
  def handle_cast(:elect, %{config: config} = state) do
    Process.send_after(
      self(),
      :elect,
      Access.get(config, :polling_interval, @default_polling_interval)
    )
  ...
  @doc false
  def handle_call(:leader, _from, %{leader: current_leader} = state),
    do: {:reply, current_leader, state}
  ...
  defp determine_election([current_leader], %{leader: current_leader} = state) do
    log(:debug, :elector, fn ->
      "No connected nodes. Leader unchanged #{inspect(current_leader)}"
    end)

    state
  end

  defp determine_election([current_node], %{leader: current_leader} = state)
       when current_node == node() do
    log(:debug, :elector, fn ->
  ...
  defp determine_election(
         nodes,
         %{leader: current_leader, config: %{strategy: strategy} = config} = state
       )
    ...
    case nodes |> strategy.leader(config) do
      ...
      %{node: ^current_leader, metadata: _} = elected ->
        reelection(elected, state)

      %{node: _elected_node, metadata: _} = elected ->
        new_leader(elected, state)
  ...
  defp reelection(%{node: _, metadata: metadata}, %{leader: current_leader} = state) do
    log(:debug, :elector, fn ->
      "Re-elected: #{inspect(current_leader)}, metadata: #{inspect(metadata)}"
    end)
